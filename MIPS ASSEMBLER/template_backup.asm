...
bubble_sort:
    	sub $t6, $t6, 1		#to get 0 like indexing i.i index starts from 0
    	move $s0, $zero
	...
	inner_loop:		
    		bge $s1, $t6, end_inner_loop	#if inner_loop has run n times, end_inner_loop
    		lw $s3, 0($t5)
    		lw $s4, 4($t5)
		ble $s3, $s4, no_swap	#if element[i]<=element[i+1], directly move to no_swap
    		sw $s3, 4($t5)
    		sw $s4, 0($t5)
